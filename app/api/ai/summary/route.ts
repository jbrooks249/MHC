...
export async function POST(req: Request) {
  try {
    ...
    const { text } = await generateText({
      ...
      prompt:
        `Write a brief investment summary for the following manufactured housing community acquisition.\n\n` +
        `${buildPropertyContext(property)}\n\n` +
        `Structure the response in markdown with these sections:\n` +
        `## Overview — 2-3 sentences on the deal at a glance (location, size, price, key metrics).\n` +
        `## Strengths — 2-3 bullet points of what makes this attractive.\n` +
        `## Risks & Diligence — 2-3 bullet points of concerns and missing data to verify.\n` +
        `## Recommendation — a clear take (Pursue / Watch / Pass) with one sentence of reasoning.\n\n` +
        `Keep the whole summary under 150 words. Be brief and decision-oriented.`,
    })
